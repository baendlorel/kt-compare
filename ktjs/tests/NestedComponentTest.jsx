...
function NestedComponentTest({ onMetrics }) {
  ...
  const updateNode = (node, element) => {
    ...
    if (textNode) {
      textNode.textContent = `Level ${node.level}: ${node.id} = ${node.value} (updates: ${node.updateCount})`;
    }

    // Update children
    if (node.children.length > 0) {
      const childElements = Array.from(element.children).slice(1); // Skip first child which is the text div
  ...
  updateButton = <button on:click={updateTree} disabled>Update All Nodes</button>;
